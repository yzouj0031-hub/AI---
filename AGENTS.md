...
**3D 只是渲染层，不许放游戏逻辑。** `R3` 里读游戏状态、写 three.js 对象，
反过来一律不行。模拟永远是 2D 的 `(x, y)`，3D 只是把它映射成 `x → X`、`y → Z`。
一旦把"谁能看见谁""怀疑度怎么算"这类判断写进 `R3`，2D 模式下就会行为不一致，
信息隔离也没人守得住了。同理，`test_meeting.py` 固定跑 2D——它测的是会议流程，
无 GPU 的环境里 3D 走软件渲染，帧率一低 `dt` 封顶会让游戏变慢动作，时序断言全失真。
渲染器本身由 `test_mobile.py` 覆盖。
